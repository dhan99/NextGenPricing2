...
## Database Tables (58 total — +1 in F1.1 for `deal_entities`, +2 in F1.2 for `assembly_templates` + `assembly_components`, +3 in F1.3 for `batch_renewal_jobs` + `batch_renewal_items` + `batch_adjustment_rules`, +1 in F1.4 for `domain_events_outbox`, +2 in F2.2 for `budget_actuals` + `budget_alerts`, +1 in F2.3 for `time_entries`, +1 in F3.1 for `collaboration_sessions`, +1 in F3.2 for `portal_invites`, +1 in F3.3 for `scope_creep_signals`, +1 in F3.4 for `voice_transcripts`, +1 in F3.6 for `rate_optimization_runs`, +1 in F4.5 for `ai_telemetry`)
