...
## Database Tables (51 total — +1 in F1.1 for `deal_entities`, +2 in F1.2 for `assembly_templates` + `assembly_components`, +3 in F1.3 for `batch_renewal_jobs` + `batch_renewal_items` + `batch_adjustment_rules`, +1 in F1.4 for `domain_events_outbox`, +2 in F2.2 for `budget_actuals` + `budget_alerts`)
...
- **Operations & governance (10)**: margin_targets, activity_log, change_orders, engagement_letters, **batch_renewal_jobs** *(F1.3)*, **batch_renewal_items** *(F1.3)*, **batch_adjustment_rules** *(F1.3)*, **domain_events_outbox** *(F1.4)*, **budget_actuals** *(F2.2)*, **budget_alerts** *(F2.2)*
