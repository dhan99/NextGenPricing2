...
## Database Tables (52 total — +1 in F1.1 for `deal_entities`, +2 in F1.2 for `assembly_templates` + `assembly_components`, +3 in F1.3 for `batch_renewal_jobs` + `batch_renewal_items` + `batch_adjustment_rules`, +1 in F1.4 for `domain_events_outbox`, +2 in F2.2 for `budget_actuals` + `budget_alerts`, +1 in F2.3 for `time_entries`)
