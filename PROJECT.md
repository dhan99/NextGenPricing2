...
> F2.1.1 added `deals.fingerprint JSONB` (nullable) — feature snapshot for the IntelligenceEngine. The matching `embedding vector(1536)` column lands in F2.1.2 once pgvector is installed in the cluster. Both columns are optional; absence/NULL means "compute on first read".
- **Operations & governance (8)**: margin_targets, activity_log, change_orders, engagement_letters, **batch_renewal_jobs** *(F1.3)*, **batch_renewal_items** *(F1.3)*, **batch_adjustment_rules** *(F1.3)*, **domain_events_outbox** *(F1.4)*
