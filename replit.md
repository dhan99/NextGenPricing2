...
shared/              - Shared code
  schema.ts          - Drizzle ORM schema (all 42 tables + relations; see `docs/audit/schema_inventory.csv` for the full list)
```
...
## Database Tables (42 total — was "13" pre-F0.7; updated 2026-05-02)

Authoritative list: `docs/audit/schema_inventory.csv` (auto-regenerated by `python3 scripts/audit/extract_schema.py`). Grouped:

- **Core domain (10)**: clients, deals, scope_catalog, scope_templates, scope_template_items, deal_scope_items, roles, rate_cards, rate_card_entries, pricing_lines
