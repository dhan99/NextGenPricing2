...
shared/              - Shared code
  schema.ts          - Drizzle ORM schema (all 43 tables + relations; see `docs/audit/schema_inventory.csv` for the full list)
```
...
## Database Tables (43 total — was "13" pre-F0.7; +1 in F1.1 for `deal_entities`)

Authoritative list: `docs/audit/schema_inventory.csv` (auto-regenerated by `python3 scripts/audit/extract_schema.py`). Grouped:

- **Core domain (11)**: clients, deals, **deal_entities** *(F1.1 — multi-entity worksheets)*, scope_catalog, scope_templates, scope_template_items, deal_scope_items, roles, rate_cards, rate_card_entries, pricing_lines
