...
async function renderLetterPdf(args: {
  ...
  doc.font("Helvetica-Bold").text(deal.title, { continued: true });
  ...
  let sx = 80, sy = cardY + 30;
  summary.forEach((row, i) => {
    const col = i % 2;
    const rIdx = Math.floor(i / 2);
    const x = 80 + col * 234;
    const y = cardY + 30 + rIdx * 18;
    doc.fillColor(STONE_500).font("Helvetica").fontSize(8).text(row[0].toUpperCase(), x, y, { width: 100 });
    doc.fillColor(STONE_900).font("Helvetica-Bold").fontSize(10).text(row[1], x + 100, y - 1, { width: 130, ellipsis: true });
  });
