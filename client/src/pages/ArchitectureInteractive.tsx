...
const nodes: NodeData[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "Intapp: screenings, hits, mitigations, intake_requests, intake_extractions, intake_approvals, intake_events, intapp_events",
  ...
  },
  {
    ...
    subtitle: "Federated onboarding + risk screening (bi-directional)",
    category: "integration",
    description: "Intapp is treated as one external system with two cooperating modules running on a parallel non-blocking track, served by two provider interfaces in code (IntappProvider for screening + outbound, IntakeProvider for intake). Intake handles federated client onboarding with AI-assisted extraction and a six-gate reviewer matrix; Screening handles conflict / independence / PEP / sanctions checks. The two tracks join exactly once at the final accept gate, and the join is enforced server-side.",
    details: [
      "Intake — auto-opened on POST /api/deals via ensureIntakeRequest()",
      "Intake — AI extraction from RFP / engagement notes (manual Apply/Dismiss today; confidence-routing is Phase-2)",
      "Intake — federated reviewer matrix: GC, Ethics, AML, Independence Partner, Jurisdictional Counsel, Pricing Committee",
      "Intake — final accept gate joins both tracks (POST /api/intake/requests/:id/accept) returns 409 if any gate is unsigned or screening is not clear",
      "Screening — seven hit types with severity (sanctions_watchlist, industry_restriction, pep, independence, conflict_of_interest, regulatory_review, fee_threshold)",
      "Screening — 5s status pill polled in deal header, mitigation lifecycle (open / resolved / waived / rejected)",
      "Outbound push (non-blocking, server-side dedupe is a forward-looking gap): autoPushIntappOutcome on deal approval/rejection; autoPushMitigation on each mitigation resolve/waive/reject event",
      "Provider abstraction (IntappProvider + IntakeProvider) — mode selects live or simulated; Live is a stub today pending INTAPP_API_TOKEN",
    ],
    technologies: ["Intapp Open API", "Intapp Intake", "Intapp OnePlace / Risk", "REST + OAuth 2.0", "X-Tenant-Id"],
    connections: [],
  },
  {
    id: "conga",
...
const dataFlows = [
  ...
  { from: "api", to: "intapp", label: "Intake + Screen + outcome push (approval) + mitigation push (lifecycle)", type: "primary" as const },
